<?php
class Alliance_JulienReviews_Adminhtml_JulienreviewsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()
            ->createBlock('alliance_julienreviews/adminhtml_report_grid')
            ->getCsvFile();
    ...
    {
        $helper = Mage::helper('alliance_julienreviews');
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('alliance_julienreviews/review');
        ...
        Mage::register('alliance_julienreviews', $model);

        $this->_initAction()
            ->_addBreadcrumb($id ? $this->__('Edit Review') : $this->__('New Review'), $id ? $this->__('Edit Review') : $this->__('New Review'))
            ->_addContent($this->getLayout()->createBlock('alliance_julienreviews/adminhtml_pending_edit')->setData('action', $this->getUrl('*/*/save')))
            ->renderLayout();
    }
    ...
    public function saveAction()
    {
        if ($postData = $this->getRequest()->getPost()) {
            $model = Mage::getSingleton('alliance_julienreviews/review');
            ...
            try {
                if ($model->save()) {
                    $review = Mage::getModel('alliance_julienreviews/review')->load($model->getId());
                    $helper = Mage::helper('alliance_julienreviews');
                    ...
                    if ($review_status == 'Approved') {
                        Mage::dispatchEvent('alliance_julienreviews_approval', $event_data);
                    }

                    if ($review_status == 'Approved' && !$review->getContributed()) {
                        $contributor = Mage::getModel('alliance_julienreviews/contributor');
                        ...
                        if ($contributor->getId()) {
                            ...
                            $new_contributor = Mage::getModel('alliance_julienreviews/contributor');
				...
				$this->_redirectUrl(Mage::helper('core')->urlDecode(Mage::getSingleton('core/session')->getJulienreviewsReferer()));
    ...
    public function deleteAction()
    {
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('alliance_julienreviews')->__('An error occurred while deleting this review.'));
        $this->_redirect('*/*/');
    }
    ...
    public function approveAction()
    {
        if ($review_id = $this->getRequest()->getParam('id')) {
                ...
                if ($model->getStatus() != 'Approved') {
                    ...
                    if ($model->save()) {
                        ...
                        Mage::getSingleton('adminhtml/session')
                            ->addError(Mage::helper('alliance_julienreviews')->__('An error occurred while trying to approve the review(s).'));
                        $this->_redirect('*/*/pending');
                    }
                }
                $helper->updateTopContributors();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('alliance_julienreviews')->__('You have successfully approved the review(s).'));
                $this->_redirect('*/*/pending');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/pending');
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('alliance_julienreviews')->__('An error occurred while trying to approve the review(s).'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function denyAction()
    {
        if ($review_id = $this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('alliance_julienreviews')->__('You have successfully denied the review(s).'));
                $this->_redirect('*/*/pending');
            }
            catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('alliance_julienreviews')->__('An error occurred while trying to deny the review(s).'));
            $this->_redirect('*/*/pending');
        }
    }
    ...
    public function massDeleteAction()
    {
        if ($review_ids = $this->getRequest()->getParam('review_ids')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('alliance_julienreviews')->__('You have successfully deleted the review(s).'));
                $this->_redirect('*/*/');
            }
            catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('alliance_julienreviews')->__('An error occurred while trying to delete the review(s).'));
            $this->_redirect('*/*/');
        }
    }
    ...
    {
        $data = Mage::getModel('alliance_julienreviews/review')->load($this->getRequest()->getParam('id'));
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('catalog/alliance_julienreviews')
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/alliance_julienreviews');
    }
}