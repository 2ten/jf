 ...
 */
class SFC_CimCore_Model_Checkout_Type_Onepage extends Mage_Checkout_Model_Type_Onepage
{
    ...
    public function savePayment($data)
    {
        ...
        if (isset($data['method']) && strpos($data['method'], SFC_CimCore_Model_Cim::METHOD_CODE_KEY_CC_LAST4) !== false) {
            $encodedMethod = $data['method'];
            $encodedMethodParts = explode(SFC_CimCore_Model_Cim::METHOD_CODE_KEY_CC_LAST4, $encodedMethod);
            ...
            // This means payment process will be handled by SFC_CimCore_Model_Cim method class and config settings
            $data['method'] = $method;
    ...
    {
        // Log
        Mage::log('Magento is saving order...', Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
        ...
        if(0 !== strpos($quote->getPayment()->getMethod(), SFC_CimCore_Model_Cim::METHOD_CODE)) {
            ...
            Mage::log('This is a CIM order, handle any exceptions by cleaning up any created CIM profiles.', Zend_Log::INFO,
                SFC_CimCore_Helper_Data::LOG_FILE);
            ...
            try {
                ...
                Mage::log('Caught order exception in order with CIM payment method.', Zend_Log::INFO,
                    SFC_CimCore_Helper_Data::LOG_FILE);
                Mage::log('Caught order exception: ' . $eOrder->getMessage(), Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                ...
                $alreadyCreatedCimCustomerProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                Mage::log('alreadyCreatedCimPaymentProfileId: ' .
                $alreadyCreatedCimPaymentProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                Mage::log('createdCimCustomerProfileId: ' .
                $createdCimCustomerProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                Mage::log('createdCimPaymentProfileId: ' .
                $createdCimPaymentProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                Mage::log('existingCimCustomerProfileId: ' .
                $existingCimCustomerProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                ...
                if (strlen($createdCimCustomerProfileId) && $createdCimCustomerProfileId != $alreadyCreatedCimCustomerProfileId) {
                    ...
                    catch (SFC_CimCore_Helper_Cim_Exception $eCim) {
                        Mage::log('Failed to delete CIM customer profile: ' .
                        $createdCimCustomerProfileId, Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                        Mage::log('Error: ' . $eCim->getMessage(), Zend_Log::INFO, SFC_CimCore_Helper_Data::LOG_FILE);
                    }
                }
                else {
