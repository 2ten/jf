 ...
 *
 */
class SFC_CimCore_Model_Sales_Quote_Payment extends Mage_Sales_Model_Quote_Payment
{
    ...
    public function addData(array $data)
    {
        // Get $data['method'] and determine if it includes last 4 cc digits
        // Check if the saved credit card payment profile data is embedded in this method code string
        if (strpos($data['method'], SFC_CimCore_Model_Cim::METHOD_CODE_KEY_CC_LAST4) !== false) {
            $encodedMethod = $data['method'];
            $encodedMethodParts = explode(SFC_CimCore_Model_Cim::METHOD_CODE_KEY_CC_LAST4, $encodedMethod);
            $method = $encodedMethodParts[0];
            $ccLastDigits = $encodedMethodParts[1];
